...
return (
    <Card sx={{
      maxWidth:'400px',minWidth:'300px', borderRadius:'15px', cursor:'pointer',transition:'300ms',
      "&:hover":{
    ...
    variant='outlined' {...props}>
      <Box sx={{padding:'15px 20px 10px 20px'}} >
        ...
        { showGrowth &&
          <Typography variant="caption" sx={{color:'secondary.main',fontWeight:'400'}} >
          {data?.symbol || "SYMBOL"}&nbsp;|&nbsp; 
          <Typography component={'span'} variant='caption' sx={{color:data?.growth?.percent[0]==='+'?'success.main':'error.main'}}>
      ...
      <Avatar sx={{width:'1.3em',height:'1.3em'}} />&nbsp;&nbsp;<Typography>{data?.basketeer || "Basketeer"}</Typography>
      </Box>
    </Card>
  );
}
